...
> [!NOTE]
> This README music player is intended/built for mobile/in-app browsers and is compatible with both light and dark modes.<br/><br/>Vertical scrolling between songs/albums (instead of using song titles/links and the navigation menu) is possible, but not recommended.<br/><br/>Audio playback is still only supported in GitHub Organization/Profile READMEs through video files, but original .wav recordings are available by request (some of this music is copyrighted- if you own the rights, please email me; if you do not, use at your own risk).
